...
| 一時的な保持 | 右クリック経路のURL1件を、そのタブのメモリに置きます。**60秒を超えた値は使いません。** 置いたときのタイマーが60秒後以降の最初の機会に参照を消し、それより前でも一度使う・別の場所を右クリックする・ページを離れると捨てられます。リンクを直接開いた回のように、その値を使わずに済んだ場合も、**開く前に、同じポストを指す控えなら消すよう試みます**（依頼が届かなかった場合も、60秒の期限・タイマー・次の右クリック・ページ移動・タブを閉じることが受け皿になります） |
...
- **提出物の作られ方** — 配布一覧と manifest の突き合わせに加えて、**実際に `tools/package.sh` を起動して**、未コミットの変更・index の印・HEAD との中身の食い違い・出力先の差し替え・途中のコマンド失敗で必ず止まること
