...
| 一時的な保持 | 右クリック経路のURL1件を、そのタブのメモリに置きます。**60秒を超えた値は使いません。** 置いたときのタイマーが60秒後以降の最初の機会に参照を消し、それより前でも一度使う・別の場所を右クリックする・ページを離れると捨てられます。リンクを直接開いた回のように、その値を使わずに済んだ場合も、開く前に消します |
