...
| ファイル | 用途 |
|---|---|
| `~/dev/postcloak-1.5.0.zip` | パッケージ（26KB・展開して main と差分ゼロを確認済み） |
...
`~/dev/postcloak-1.5.0.zip` を選ぶ。

> アップロードすると、名前と説明は ZIP 内の `manifest.json` の値に置き換わる。名前欄を手で入力する必要はない。
> 反映後の表示: **`PostCloak — x.comのポストをシークレットウィンドウで開く`** / バージョン **1.5.0**
>
...
現行アイコンは「濃いグレーの円＋灰色のフェドーラ帽＋黒いサングラス＋オレンジの南京錠」です。X ロゴとの類似はありませんが、**帽子＋サングラスは Chrome 自身のシークレットモードのアイコンとほぼ同じ意匠**です。2026-08-03 に、一次資料と実例で調べました（31体の並列調査＋敵対検証）。

### 結論: 差し替えを推奨する。ただし「審査に落ちるから」ではない

**審査で落ちる可能性は、調べた範囲では低い**というのが実測の結論です。それでも差し替えを推奨する理由は別にあります。
...
**3. 灰色を回避するコストが、いま実質ゼロ**

未提出でユーザーが0人なので、原本を1枚描き直して3サイズ書き出すだけです。公開後に指摘されて差し替えるのとは、識別性の喪失という点で意味が違います。
...
**推奨は A。** 16px に耐え、名前と意味が一致し、既存10件のどれとも重なりません。明快さを最優先するなら C。

画像: `~/dev/postcloak-store-assets/icon-candidates/`（`candidates-128px.png` / `candidates-at-16px.png` / `candidates-source.html`）

### 差し替えないと決めた場合

そのまま提出して構いません。その場合でも、**16px の可読性だけは別途手を入れる価値があります**（南京錠を外して帽子とサングラスの2要素に減らし、コントラストを上げる等）。
