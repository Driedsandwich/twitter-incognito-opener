...
| **ウェブサイトのコンテンツ** | 例: テキスト、画像、音声、動画、**ハイパーリンク** | DOM から読むのは `getAttribute('href')` の**2箇所だけ**＝**ハイパーリンク**。テキスト・画像・音声・動画は読まない（`innerText` / `innerHTML` / `src` はいずれも0件） | **チェックする。** 定義の例に「ハイパーリンク」が名指しされており、端末内だけの処理でも開示が要る（FAQ） |
